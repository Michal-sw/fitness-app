...
export const addActivity = async ({
  ...
  for (const user of attendees) {
    ...
    if (userResult.statusCode !== 200)
      return getErrorObject(404, "Activity not added to user");
  }
...
export const getActivityById = async (id: string) => {
  const activity = await Activity.findById(id).populate("attendees", "login");
  if (!activity) {
...
export const getActivitiesByUser = async (userId: string) => {
  ...
  const activities = await Activity.find({ attendees: id }).populate(
    "attendees",
    "login"
  );
  if (!activities) {
